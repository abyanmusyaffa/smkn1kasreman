<article class="flex w-full flex-col {{ $index % 2 !== 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'}} items-center lg:justify-between rounded-2xl bg-white p-4 gap-4 lg:px-24 lg:py-6">
    ...
    <figcaption class="text-center lg:text-start w-full lg:w-1/2 flex flex-col gap-8 lg:gap-14 items-center {{ $index % 2 !== 0 ? 'lg:items-end' : 'lg:items-start'}}">
        <div class="flex flex-col gap-2 lg:gap-4">
            <div class="flex flex-col items-center {{ $index % 2 !== 0 ? 'lg:items-end lg:text-end' : 'lg:items-start lg:text-start'}} ">
                ...
                <h3 class="font-semibold text-2xl lg:text-4xl text-slate-800">{{ $name }}</h3>
            </div>
        ...
        <button wire:click="dispatch('open', { description: '{{ $description }}', logo: '{{ $logo }}', alias: '{{ $alias }}', name: '{{ $name }}' })" class="bg-blue-600 flex w-fit items-center gap-1 px-2 py-1 lg:gap-2 lg:px-4 lg:py-2 rounded-lg hover:bg-blue-700">
